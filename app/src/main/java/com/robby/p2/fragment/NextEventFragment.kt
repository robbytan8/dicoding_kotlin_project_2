...
class NextEventFragment : EventTabFragment() {

    override fun initMatchData(leagueId: String) {
        val queue = Volley.newRequestQueue(ctx)
        val stringRequest = StringRequest(Request.Method.GET, TheSportDBApi().getNextEventsInLeague(leagueId),
            Response.Listener<String> { response ->
                ...
                if (!jsonObject.isNull("events")){
                    ...
                    view?.let { snackbar(it, "There are no next event match in this league") }
                }
